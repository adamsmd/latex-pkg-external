...
compiling \LaTeX\ code in a separate document that has its own
preamble.
...
For example, in the following it is nearly impossible to tell the
difference between the ``B'' generated from \com{external} and the one
that was not.
...
There are a number of packages the provide similar functionality to
that of \Pkg{external}.
The main distinguishing features \Pkg{external} is that it allows
...
Allows extracting certain environment from \LaTeX\ sources as graphics.
...
in the master file.
As with Section~\ref{subsec:Ignoring Preambles}, these packages are
geared towards helping authors manage large documents that otherwise
take a long time to compile (e.g., a book with each chapter as a
sub-document).
When compiling the master document, these packages do not compile the
sub-documents separately.
Thus, they do not support separate preambles when compiling the master
...
Allows combining documents where each sub-document is a complete normal
\LaTeX\ document that is typeset separately.
...
There are several packages for displaying code listings next to the
results of that code.
These include \pkg{tcolorbox}, \pkg{example}, \pkg{examplep},
\pkg{latexdemo}, and \pkg{showexpl}.
They all provide environments display their content as source code next
to the result of rendering that code.
...
Provides the document class |standalone|, which produces a page with
...
each piece of externally compiled code.
...
used by the \LaTeX\ file that is generated for externally compiled
code.
...
externally compiled code in an \com{external} command or
...
output by the externally compiled code in an \com{external} command or
The following is an example of this in action.
