...
\tcbset{docexample/.style={colframe=ExampleFrame,colback=ExampleBack,
...
\begin{abstract}
The \Pkg{external} package allows you to include the result of
rendering \LaTeX\ code in a separate document that has its own
preamble.
...
rendered results in a master document, this allows you to use both
...
the following.
(The only difference between the \Com{external} command and the

\env{externalenv} environment is that one is a command and the other is
an environment.)
\begin{dispExample}
An atom undergoing
...
Note that even though these examples are rendered as separate
...
Main difference is ability to specify own preamble and
exact sizing

\paragraph{\pkg{childdoc}} TODO:~describe

\paragraph{\pkg{combine}} TODO:~describe

\paragraph{\pkg{docmute}} TODO:~describe

\paragraph{\pkg{includex}} TODO:~describe

\paragraph{\pkg{minidocument}} TODO:~describe

\paragraph{\pkg{newclude}} TODO:~describe

\paragraph{\pkg{pgfplots}} TODO:~describe

\paragraph{\pkg{preview}} TODO:~describe

\paragraph{\pkg{standalone}} TODO:~describe

\paragraph{\pkg{subdocs}} TODO:~describe

\paragraph{\pkg{subfiles}} TODO:~describe

\paragraph{\pkg{tcolorbox}} TODO:~describe

\paragraph{\pkg{tikz}} TODO:~describe

\paragraph{\pkg{example}} TODO:~describe

\paragraph{\pkg{examplep}} TODO:~describe

\paragraph{\pkg{latexdemo}} TODO:~describe

\paragraph{\pkg{showexpl}} TODO:~describe
...
It is sometimes useful to define versions of \com{external} and
\env{externalenv} that have default values for their options.
...
With \Com{newexternal}, one can define a version of \com{external} that has default values for its options.
...
With \Com{newexternalenv}, one can define a version of \env{externalenv} that has default values for its options.
...
This option specifies (by way of \Com{documentclass}) the document
class to be used by the intermediate \LaTeX\ file that is generated for
each piece of externally rendered code.
...
used by the \LaTeX\ file that is generated for externally rendered
code.
...
intermediate \LaTeX\ file that is generated for externally rendered
code.
...
This option specifies \LaTeX\ code to run before compiling the
externally rendered code in an \com{external} command or
\env{externalenv} environment.
The following is an example of this in action.
...
This option specifies \LaTeX\ code to run before reading the file
output by the externally rendered code in an \com{external} command or
The following is an example of this in action.
...
generated for externally rendered code.
...
\begin{docKey}{file/pdf}{=\meta{extension}}{initially |.pdf|}
...
rendering the \LaTeX\ code that is rendered separately.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{docCommand}{ExternalCode}{\marg{dimension~file}\marg{preamble}\marg{before~savebox}\marg{body}\marg{before~usebox}}
...
rendered contains an error that causes the compilation of the
...
\label{subsec:Hashes and Command Arguments}
