...
The only difference is that \com{localpreamble} is a command and
\Env{localpreambleenv} is an environment.
...
\item reading the file that results from that compilation (which is
  probably a PDF file).
\end{enumerate}
These are handled with the commands \Com{LocalPreambleWrite},
...
dependencies.
Thus in the previous example, using |\RequirePackage{locpream.code}|
instead of |\RequirePackage{locpream}| minimizes the compilation time.
...
\pkg{article}) contains the \Com{pagename} macro.\marginpar{TODO:REV}
...
%%%%%%%%%%%%%%%%%%%%
\begin{docKey}{math}{=\meta{\docValue{false}, \docValue{inline}, or
  \docValue{display}}}{initially \docValue{false}}

This option controls whether the body of \com{localpreamble} or
\env{localpreambleenv} is treated as math, and if so, whether it is
...
With a \com{localpreamble} command or \env{localpreambleenv}
environment, these options specify \LaTeX\ code to run before or after
\cmd{LocalPreambleWrite}, \cmd{LocalPreambleCompile}, or
\cmd{LocalPreambleRead}.
The following is an example of these in action.\marginpar{missing ``l''}
...
This even applies in the body of an \com{localpreamble} or
\env{localpreambleenv} as seen in the following.
